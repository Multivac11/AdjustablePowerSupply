...
{
    if (queue == nullptr || listener_count_ >= PERF_MAX_LISTENERS) return false;
...
void PerfMonitor::Run()
{
    ...
    while (true)
    {
        ...
        if (elapsed <= 0) elapsed = interval_ms_ * 1000ULL;
        ...
        // ESP_LOGI(TAG, "cpu0=%d, cpu1=%d, free_heap=%d", cpu0, cpu1, event_.free_heap_);
